...
describe('Span status follows OTel HTTP semantic conventions (#77)', () => {
	...
	it('should set span status ERROR for 500 from thrown error', async () => {
		const app = new Elysia()
			.use(opentelemetry({ serviceName: 'semconv-500-test' }))
			.get('/server-error', () => {
				throw new Error('Internal failure')
			})

		const response = await app.handle(req('/server-error'))
		expect(response.status).toBe(500)
		...
		const rootSpans = spans.filter((s) =>
			s.name.startsWith('GET /server-error')
		)
		if (rootSpans.length > 0) {
			expect(rootSpans[0].status.code).toBe(SpanStatusCode.ERROR)
		}
	})

	it('should set span status ERROR when onError keeps 5xx', async () => {
		const app = new Elysia()
			.use(opentelemetry({ serviceName: 'semconv-503-test' }))
			.onError(({ error, set }) => {
				set.status = 503
				return { error: error.message }
			})
		...
		const response = await app.handle(req('/unavailable'))
		...
		const rootSpans = spans.filter((s) =>
			s.name.startsWith('GET /unavailable')
		)
		if (rootSpans.length > 0) {
			expect(rootSpans[0].status.code).toBe(SpanStatusCode.ERROR)
		}
